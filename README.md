...
Methods read from the `$_SERVER` superglobal. Alternatively a string containing a header (with or without field name) or a complete HTTP response can be supplied as an argument. Methods will return `false` if the request header is not present or cannot be parsed.
